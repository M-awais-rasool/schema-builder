import { tableFieldHeight, tableHeaderHeight } from "./constants";
 ...
 */
export function calcPath(r: { startTable: { x: any; y: number; }; startFieldIndex: number; endTable: { x: any; y: number; }; endFieldIndex: number; }, tableWidth: number = 200, zoom: number = 1): string {
  if (!r) {
    return "";
  }
